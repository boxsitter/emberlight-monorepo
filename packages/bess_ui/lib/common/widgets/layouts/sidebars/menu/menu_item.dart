        ...
        () => Padding(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: menuController.isHovering(route) || menuController.isActive(route) ? BessColors.primary : Colors.transparent,
            ...
            child: Row(
              ...
              children: [
                //Icon
                Padding(
                  padding: const EdgeInsets.only(left: BessSizes.lg, top: BessSizes.md, bottom: BessSizes.md, right: BessSizes.md),
                  child: menuController.isActive(route)
                      ? Icon(icon, size: 22, color: BessColors.white)
                      : Icon(icon, size: 22, color: menuController.isHovering(route)
                      ? BessColors.white
                      : BessColors.darkGrey),
                ),
                ...
                if (menuController.isHovering(route) || menuController.isActive(route))
                  Flexible(child: Text(itemName, style: Theme.of(context).textTheme.bodyMedium!.apply(color: BessColors.white)))
                else
                  Flexible(child: Text(itemName, style: Theme.of(context).textTheme.bodyMedium!.apply(color: BessColors.darkGrey))),
              ],
            )
          ),
        ),
